 ...
 * cuts are handed back in reading order, so a run's colour rides along with every word out
 * of it, and a name of two words is two words rather than one long one.
 *
...
/** One word of it, or the gap after one. */
export interface TypedPart extends TypedRun {
 ...
 */
export function typewriterParts(runs: readonly TypedRun[]): TypedPart[] {
	const parts: TypedPart[] = [];
	let index = 0;
	for (const run of runs) {
		for (const piece of run.text.match(/\s+|\S+/g) ?? []) {
			const word = !/^\s+$/.test(piece);
			parts.push({ text: piece, color: run.color, word, index: word ? index++ : -1 });
		}
	}
	return parts;
}
