<script lang="ts">
	 ...
	 * `calc()`: the canvas is eight squares across ({@link GROUND_FIELD_COLUMNS} — three
	 * columns of cells at three squares a cell, less the one column the board is drawn
	 * without) and eleven down — the board's thirteen (four rows of cells at three squares a
	 * cell, plus the apron), less the two the brow takes off the top — so one square is
	 * `min(100vw/8, 100dvh/11)`, the same two limits the canvas sizes itself between. That is
	 * what makes a square here exactly a square there. The sheet is sixteen tiles across, so
	 * drawing one tile at one square is a background sixteen squares wide with the wanted
	 * tile's column scrolled into view.
	 */
	 ...
	 * count is `grid-cols-8` below, a class being a literal.) */
	...
	/**
	 * How many rows to draw. The band under the board is `100dvh − 11 squares`, which at eight
	 * squares to the width is `8 × dvh/vw − 11` rows, so this many covers a view up to
	 * `(rows + 11) / 8` times as tall as it is wide — near three, which is past the narrowest
	...
	const SQUARE =
		'size-[min(100vw/8,100dvh/11)] [background-image:url(/assets/tiles/grass-16x16.png)] [background-size:calc(16*min(100vw/8,100dvh/11))_calc(16*min(100vw/8,100dvh/11))] [image-rendering:pixelated]';
	...
	const EARTH_COLUMN_CLASSES: Record<number, string> = {
		6: '[background-position:calc(-6*min(100vw/8,100dvh/11))_0]',
		7: '[background-position:calc(-7*min(100vw/8,100dvh/11))_0]',
		8: '[background-position:calc(-8*min(100vw/8,100dvh/11))_0]'
	};
