<script lang="ts">
	...
	// Whether the game's own furniture stands around the map, and it is the whole of what the
	// two routes drawing this differ by.
	//
	// This is the map and what is at the place it is open on — the terrain, the strips laid over
	// it, and the block naming the open place — plus the machinery that answers for both: the
	// polygons, the region tree, the shows every town flies, the holders, the sieges, the packs,
	// and the sheets those presses raise. None of that divides in two, which is why the map and
...
<!-- (The splash is not here. It is the front door's and no other route's — the shell takes
	it down itself anywhere else — so it stands on `/`'s own page beside this, rather than
	travelling with a component two addresses mount. See SplashScreen.) -->
